...
describe 'compiled component rds-proxy' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/postgres.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/postgres/rds-proxy.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "RdsProxy" do
      ...
      it "to have property EngineFamily" do
          expect(resource["Properties"]["EngineFamily"]).to eq("POSTGRESQL")
      end
