...
describe 'compiled component rds-proxy' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/disable_iam_auth.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/disable_iam_auth/rds-proxy.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "RdsProxy" do
      ...
      it "to have property Auth" do
          expect(resource["Properties"]["Auth"]).to eq([{"AuthScheme"=>"SECRETS", "IAMAuth"=>"DISABLED", "SecretArn"=>{"Ref"=>"SecretCredentials"}}])
      end
